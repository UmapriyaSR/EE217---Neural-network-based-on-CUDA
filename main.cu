...
REAL *d_Sunspots;
__global__ void normalizeKernel(REAL *d_Sunspots, REAL Max, REAL Min)
{
  
    int Year = threadIdx.x + blockIdx.x * blockDim.x;
    if (Year < NUM_YEARS)
    {
        d_Sunspots[Year] = ((d_Sunspots[Year]-Min) / (Max-Min)) * (HI-LO) + LO;
    }
}

__global__ void optimizedReduction(REAL *out, REAL *in, unsigned size)
{
    /********************************************************************
...
__global__ void findMinMax(REAL *array, REAL *results, int n) {
    int index = threadIdx.x + blockIdx.x * blockDim.x;
    int stride = gridDim.x * blockDim.x;
    __shared__ REAL sharedMin[256];
    __shared__ REAL sharedMax[256];

    REAL localMin = MAX_REAL;
    REAL localMax = INT_MIN;
    ...
    for (int i = blockDim.x / 2; i > 0; i >>= 1) {
        if (threadIdx.x < i) {
            sharedMin[threadIdx.x] = MIN(sharedMin[threadIdx.x], sharedMin[threadIdx.x + i]);
            sharedMax[threadIdx.x] = MAX(sharedMax[threadIdx.x], sharedMax[threadIdx.x + i]);
        }
        __syncthreads();
    }
    ...
    if (threadIdx.x == 0) {
        results[blockIdx.x] = sharedMin[0];
        results[blockIdx.x + gridDim.x] = sharedMax[0];
    }
}
...
void NormalizeSunspots()
{
  ...
  REAL* results;
  REAL* d_results;
  results = (REAL *)malloc(2 * ((NUM_YEARS + 255) / 256) * sizeof(REAL));
  cudaMalloc((void **)&d_results, 2 * ((NUM_YEARS + 255) / 256) * sizeof(REAL));
  Min = MAX_REAL;
  Max = MIN_REAL;
  findMinMax<<<(NUM_YEARS + 255) / 256, 256>>>(d_Sunspots, d_results, NUM_YEARS);
  cudaMemcpy(results, d_results, 2 * ((NUM_YEARS + 255) / 256) * sizeof(REAL), cudaMemcpyDeviceToHost);
  for (int i = 0; i < (NUM_YEARS + 255) / 256; i++) {
        Min = MIN(Min, results[i]);
        Max = MAX(Max, results[i + (NUM_YEARS + 255) / 256]);
  }
  free(results);
  cudaFree(d_results);
  ...
  // Add code for Mean
  normalizeKernel<<<(NUM_YEARS + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE>>>(d_Sunspots, Max, Min);
  ...
  out_elements = NUM_YEARS / (BLOCK_SIZE<<1);
  if(NUM_YEARS % (BLOCK_SIZE<<1)) out_elements++;

  dim3 dim_grid, dim_block;
  REAL  *out_h, *out_d;
  out_h = (REAL*)malloc(out_elements * sizeof(REAL));
  cudaMalloc((void**)&out_d, out_elements * sizeof(REAL));
  cudaMemset(out_d, 0, out_elements * sizeof(REAL));
  dim_block.x = BLOCK_SIZE; dim_block.y = dim_block.z = 1;
  dim_grid.x = out_elements; dim_grid.y = dim_grid.z = 1;
  optimizedReduction<<<dim_grid, dim_block>>>(out_d, d_Sunspots, NUM_YEARS);
  cudaMemcpy(out_h, out_d, out_elements * sizeof(REAL), cudaMemcpyDeviceToHost);
  for(int i=1; i<out_elements; i++) {
		out_h[0] += out_h[i];
	}
  Mean=out_h[0]/NUM_YEARS;
  cudaFree(out_d);
  free(out_h);
...
__global__ void propagateLayerKernel(REAL *lowerOutput, REAL *upperOutput, 
                                     REAL *upperWeight,
...
void PropagateLayer(NET* Net, LAYER* Lower, LAYER* Upper) {
    ...
    cudaMalloc(&d_lowerOutput, (Lower->Units + 1) * sizeof(REAL));
    cudaMalloc(&d_upperOutput, (Upper->Units + 1) * sizeof(REAL));
    cudaMalloc(&d_upperWeight, (Upper->Units + 1) * (Lower->Units + 1) * sizeof(REAL));
   
    cudaMemcpy(d_lowerOutput, Lower->Output, (Lower->Units + 1) * sizeof(REAL), cudaMemcpyHostToDevice);
    cudaMemcpy(d_upperWeight, Upper->Weight, (Upper->Units + 1) * (Lower->Units + 1) * sizeof(REAL), cudaMemcpyHostToDevice);
    //cudaMemcpy(d_upperOutput, Upper->Output, (Upper->Units + 1) * sizeof(REAL), cudaMemcpyHostToDevice);
    
    // Define grid and block dimensions
    dim3 blockDim(BLOCK_SIZE);
    dim3 gridDim((Upper->Units + BLOCK_SIZE) / BLOCK_SIZE);
